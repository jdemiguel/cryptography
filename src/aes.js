...
class AES {
  ...
  subBytes(input) {
    return input.map(item => this.applyTable(item, SUBWORD))
  }
  shiftRows(input) {
  ...
  invSubBytes(input) {
    return input.map(item => this.applyTable(item, INVERSE_SUBWORD))
  }
  invShiftRows(input) {
    const output = Array(4).fill(BigInt(0))
    for (let i = 0; i < 4; i++) {
  ...
  invMixColumns(input) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (let j = 0; j < 4; j++) {
        outputVector[j] =
          multiplyBytes(INV_MIX_COLUMNS[j][0], inputVector[0]) ^
          multiplyBytes(INV_MIX_COLUMNS[j][1], inputVector[1]) ^
          multiplyBytes(INV_MIX_COLUMNS[j][2], inputVector[2]) ^
          multiplyBytes(INV_MIX_COLUMNS[j][3], inputVector[3])
  ...
  getKeys(key) {
      ...
      for (let i = this.nk; i < 28 + this.nk * 4; i++) {
        ...
        if (i % this.nk == 0) {
          ...
          temp = this.applyTable(temp, SUBWORD)
          temp = temp ^ this.getRc(i)
        }
        if (this.nk > 6 && i % this.nk == 4) {
          temp = this.applyTable(temp, SUBWORD)
        }
        temp = temp ^ w[i - this.nk]
        w.push(temp)
      }
   ...
   * @param {*} input BigInt
   */
  applyTable(input, table) {
    let output = BigInt(0)
    for (let index = 3; index >= 0; index--) {
      const tmp = (input >> BigInt(index * 8)) & MASK_8
      output += BigInt(table[tmp]) << BigInt(index * 8)
    }
    return output
  }
...
function multiplyBytes(a, b) {
  switch (a) {
    case 1:
      return b
    case 2:
      return BigInt(MULTIPLY_2[b])
    case 3:
      return BigInt(MULTIPLY_3[b])
    case 9:
      return BigInt(MULTIPLY_9[b])
    case 11:
      return BigInt(MULTIPLY_11[b])
    case 13:
      return BigInt(MULTIPLY_13[b])
    case 14:
      return BigInt(MULTIPLY_14[b])
  }
}
